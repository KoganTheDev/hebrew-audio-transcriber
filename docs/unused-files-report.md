...
**Date:** 2026-08-18. **Reviewed and updated 2026-08-22**, after the formatting/asset split below - see that section for what changed and what this pass corrected.
**Branch:** `feature/transcript-help-and-tour`
**Scope:** tracked source tree (`git ls-files`, 137 files at the original pass) plus untracked/ignored on-disk clutter. `.claude/worktrees/` was excluded from all greps - it is a stale duplicate checkout and doubles every hit. As of the 2026-08-22 pass `.claude/` is gitignored outright (see below), so this exclusion is now automatic rather than a manual grep workaround.

The tracked source tree is clean: every Python module is imported somewhere (checked all 24 non-test, non-`__init__` modules by grepping for `import`/`from` references), all 60 vista images are consumed via a directory glob (`_vista_names()` in `core/formatting/assets.py`, moved there from `formatting.py` by the package split), and there are no `*_old`, `*.bak`, or "copy" files anywhere in the tracked set - only the word "copy" appearing in prose (UI strings, comments, LICENSE boilerplate). The real deletable/reclaimable material lives outside git: multi-gigabyte model/asset caches and test artifacts. On the tracked side, every finding from the original pass has since been resolved (see "Actions taken" below) except `run.bat`/`run.ps1` staying undocumented - the report has no open tracked-deletion candidate left.

## Verdict summary

- **Tracked deletion candidates:** none outstanding. The mockup folder and the shadowed `pyproject.toml` pytest block that this report originally flagged are both gone - see "Actions taken". The one open item is a documentation gap, not a deletion candidate: `run.bat`/`run.ps1` still aren't mentioned in the README (recommendation 6, below).
- **Untracked/ignored clutter, reclaimable:** roughly **6.36 GB** total at the original pass - `whisper_models/` (5.9 GB) + `vistas_source/` (110 MB) + `diarization_models/` (36 MB) + `mp3_test/` (305 MB, privacy issue, since fixed) + coverage/cache/log artifacts (~2.7 MB). `.claude/worktrees/agent-ae356780d22b30f13` (7.1 MB), counted separately here at the original pass, is confirmed still present on disk as of 2026-08-22 - it was never actually deleted, only newly covered by the `.claude/` gitignore rule added since (see below), which makes it harmless to tracked-tree greps even though it is still sitting there. Deleting it (`git worktree remove`, per recommendation 1) is still open if the disk space matters.
- **Privacy flag:** `mp3_test/` (305 MB of real recordings) is now excluded by directory name in `.gitignore`, not just by file extension - see "Actions taken".
...
- **Done - `docs/mockups/` deleted** (7 files, ~288 KB). The four comments in `transcript.css` that named those files by path (the design register, the palette choice, the backdrop opacity, the squircle control style) were rewritten first, so each still records *why* the decision went the way it did without pointing at a file that no longer exists. The originals remain in git history. (`transcript.css` itself no longer exists as one file - see "Since this report" below; the rewritten comments now live across `core/assets/css/00-tokens.css`, `16-toolbar.css` and `40-source.css`.)
...
- Suite green throughout at **344 passed, 1 skipped** *(at the time of this pass - see "Since this report" for the current count, which has moved for unrelated reasons)*.

## Since this report (2026-08-22): the formatting/asset split

None of this report's findings were invalidated by the split - it is noted here only because the report's own file paths and pytest count are now stale:

- `core/assets/transcript.css` / `transcript.js`, the two files this report's "confirmed used" section names as packaged via `setup.py`, no longer exist. Each is now a directory of numbered fragments - `core/assets/css/00-tokens.css` ... `90-responsive.css`, `core/assets/js/00-preamble.js` ... `99-init.js` - concatenated at render time by `_asset_dir()` in `core/formatting/assets.py`. Every fragment is read via that same directory glob, so none is individually identifiable as dead by the reference-counting method this report used elsewhere; the whole `css/`/`js/` pair is the unit of use, same as the vista photos already were.
- `core/formatting.py`, the single ~1300-line module this report's "confirmed used" section also names, is now the package `core/formatting/` (`__init__.py`, `timecode.py`, `turns.py`, `assets.py`, `chrome.py`, `document.py`). All six files are imported from `__init__.py` or from each other; none is unused.
- `core/progress_scale.py` is new since this report and is imported by both `core/transcriber.py` and `core/worker.py` - not a dead file.
- `tests/js/` (11 `*.test.mjs` files plus `harness.mjs` and `render_fixture.py`), `package.json` and `package-lock.json` are new since this report. All are exercised: `node --test "tests/js/*.test.mjs"` runs them directly, and `tests/test_js_behaviour.py` (also new) runs the same command from `pytest`, skipping with a stated reason when Node or `node_modules/` isn't available rather than failing the whole suite. `node_modules/` is gitignored; `package-lock.json` is tracked.
- `tests/test_layering.py` and `tests/test_i18n_keys.py` are also new since this report - both are guard tests (no `core/` import of PyQt5/`gui.i18n`; every i18n key `core/` emits exists in `STRINGS`), not dead weight.
- The suite count this report recorded (**344 passed, 1 skipped**) no longer matches current `HEAD` (`py -3 -m pytest -q` now reports **325 passed, 1 skipped**, plus **57 passed** from `node --test "tests/js/*.test.mjs"` run separately) - the difference is net test churn across the commits between this report and now, not a regression introduced by the split itself.
